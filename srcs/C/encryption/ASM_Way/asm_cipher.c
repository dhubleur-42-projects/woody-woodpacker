...
/*   Updated: 2023/05/16 20:12:21 by jmaia            ###   ###               */
/*                                                                            */
/* ************************************************************************** */
...
{
	{
		uint8_t message[48] = "Le chat est mort avec les vaches";
		uint8_t key[] = "J'adore les chips en ete avec du";
		uint8_t iv[] = "Idem,c'est top !";
		uint8_t encrypted[48];

		bzero(encrypted, 48);
		aes_cbc_encrypt(message, 32, key, iv, encrypted);
		for (int i = 0; i < 48; i++)
			printf("%02x ", encrypted[i]);
	}
	printf("\n");
	{
		uint8_t message[] = "Le chat est mort avec les vaches";
		uint8_t key[] = "J'adore les chips en ete avec du";
		uint8_t iv[] = "Idem,c'est top !";
		uint8_t encrypted[48];

		bzero(encrypted, 48);
		cipher(message, 32, key, iv, encrypted);
		for (int i = 0; i < 48; i++)
			printf("%02x ", encrypted[i]);
	}
}
