...
/*   Updated: 2023/11/24 16:58:20 by dhubleur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Elf32_Phdr *find_code_cave_elf32(t_file_elf32 file_elf32, size_t payload_size) {
	for (int i = 0; i < file_elf32.header->e_phnum - 1; i++) {
		if (file_elf32.programs[i].p_type != PT_LOAD)
			continue;
		if (!(file_elf32.programs[i].p_flags & PF_X))
			continue;
		if (file_elf32.programs[i + 1].p_type != PT_LOAD)
			continue;
		size_t available_space = file_elf32.programs[i + 1].p_offset - (file_elf32.programs[i].p_offset + file_elf32.programs[i].p_memsz);
		if (available_space < payload_size)
			continue;
		return &(file_elf32.programs[i]);
	}
	return NULL;
}