...
/*   Updated: 2023/11/23 15:44:14 by dhubleur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	extend_and_shift_elf64(size_t payload_length, char *map, size_t map_length, t_options options) {
	...
	for (int i = 0; i < file_elf64.header->e_phnum - 1; i++) {
		...
		if (file_elf64.programs[i + 1].p_type != PT_LOAD)
			continue;
		code_cave_segment_idx = i;
		break;
	}
	if (code_cave_segment_idx == -1) {
		return;
	}

	if (options.verbose)
		printf("Extend segment at 0x%.8lx and use it as a code cave\n", file_elf64.programs[code_cave_segment_idx].p_offset);

	off_t last_address_used = file_elf64.programs[code_cave_segment_idx].p_offset + file_elf64.programs[code_cave_segment_idx].p_memsz + payload_length;
	size_t bytes_used_to_align = file_elf64.programs[code_cave_segment_idx + 1].p_align - last_address_used % file_elf64.programs[code_cave_segment_idx + 1].p_align;
	off_t new_offset = last_address_used + bytes_used_to_align;
	size_t old_offset = file_elf64.programs[code_cave_segment_idx + 1].p_offset;
	...
	file_elf64.programs[code_cave_segment_idx + 1].p_vaddr += shifting;

	file_elf64.programs[code_cave_segment_idx].p_flags |= PF_X;
	...
	{
		file_elf64.programs = (Elf64_Phdr *)(map + file_elf64.header->e_phoff);
		size_t available_space = file_elf64.programs[code_cave_segment_idx + 1].p_offset - (file_elf64.programs[code_cave_segment_idx].p_offset + file_elf64.programs[code_cave_segment_idx].p_memsz);
		printf("New available space: %lu (between 0x%.8lx and 0x%.8lx)\n", available_space, file_elf64.programs[code_cave_segment_idx].p_offset + file_elf64.programs[code_cave_segment_idx].p_memsz, file_elf64.programs[code_cave_segment_idx + 1].p_offset);
	}
}